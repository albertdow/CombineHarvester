...
#include <vector>
//
void PlotFit(TString workspace = "etau_htt_bkg_et",
  ...
  TString shapesDir = WS + "_prefit";
  if (postFit) {
    shapesDir = WS + "_postfit";
  }
  if (WS.Contains("2016"))
  ...
  TH1F * histGraph = (TH1F*)mlfit->Get(shapesDir+"/data_obs");
  ...
  TH1F * TOTAL_    = (TH1F*)mlfit->Get(shapesDir+"/TotalBkg"); 
  ...
  histData->SetTitle("");
  for (int iB=0; iB<nBins; ++iB) {
    histData->SetBinContent(iB+1,histGraph->GetBinContent(iB+1));
    histData->SetBinError(iB+1,histGraph->GetBinError(iB+1));    
  }
  ...
  std::cout << "err = " << 1.0/TMath::Sqrt(histData->GetSumOfWeights()) << std::endl;
  ...
  */
  ...
  canv1->Update();
  if (postFit) {
    canv1->Print("figures_stat/"+WS+"_multidim_postFit.png");
    //    canv1->Print("figures_stat/"+WS+"_postFit.pdf","pdf");
  }
  else {
    canv1->Print("figures_stat/"+WS+"_multidim_preFit.png");
